    ...
    var resultsDis =  newsList.map((article,i) => (
      ...
      <div  style={{display:'flex',justifyContent:'center'}}>
        <Card
          ...
          actions={[
              <Icon type="read" onClick={() => openModal(article.title, article.content)} key="ellipsis2" />,
              <Popover content={
...
//modal
var openModal = (toTitle, toText) => {
  var toTextDis = [toText];
  setModal({title: toTitle, text: toTextDis})
  setVisible(true);
}
...
return (
    <div>
      ...
      {totalResultDis}
      <Spin style={{position:"absolute"}} size="large" spinning={isLoading}>
        <div className="Card">
          {resultsDis}
        </div>
      </Spin>
